document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.product-add-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            btn.innerHTML = '<svg viewBox="0 0 24 24" style="width:20px;height:20px;stroke:currentColor;fill:none;stroke-width:2;"><polyline points="20 6 9 17 4 12"/></svg> Dodano!';
            btn.style.background = '#22c55e';
            setTimeout(() => {
                btn.innerHTML = originalContent; // Restore original content including SVG
                if (!originalContent.includes('svg')) {
                     // Fallback if original content capture failed or was simple text
    ...
    if (newsletterSubmit) {
        newsletterSubmit.addEventListener('click', function() {
            ...
            if (input && input.value) {
                alert('Dziekujemy za zapisanie sie do newslettera! Twoj kod rabatowy: TOYLAND10');
                input.value = '';
            }
        });
    }
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            e.preventDefault();
            alert('Dziekujemy za wiadomosc! Odezwiemy sie wkrotce.');
            this.reset();
        });
    }
});
